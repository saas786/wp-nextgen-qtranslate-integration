...
/* qTranslate Integration - Start */
;(function($){
	
	function buildTranslatedString( areas ){
		var result = '';
		
		$(areas).each(function(){
			var area = $(this);
			/* TODO qTranslate escape */
			result += '<!--:' + area.data('lang') + '-->' + $.trim(area.val()) + '<!--:-->';
		});
	...
	/* Onload */
	$(function(){
		var picsTranslateBoxes = $('.qtranslate-desc-boxes');
		
		picsTranslateBoxes.each(function(){
			var translateBox = $(this);
			var langTextareas = translateBox.find('textarea');
			
			langTextareas.bind('keyup change', function(){
				var result = buildTranslatedString( langTextareas );
				var realTextarea = translateBox.siblings('textarea:first');
				
				/* Update the real textarea */
				realTextarea.val( result );
			});
		});
	});
})(jQuery);
/* qTranslate Integration - End */
			...
			foreach($image_columns as $image_column_key => $column_display_name) {
				...
				switch ($image_column_key) {
					...
					case 'alt_title_desc' :
						...
						<td <?php echo $attributes ?>>
							<?php 
							/* Support translated description for each registered language */
							if( function_exists( 'qtrans_getSortedLanguages' )) :
								global $q_config;
								$q_langs = qtrans_getSortedLanguages();
								if( is_array( $q_langs ) && count( $q_langs ) > 0 ) : $q_translate_boxes = true; ?>
								<div class="qtranslate-desc-boxes">
									<?php foreach( $q_langs as $q_lang ) : ?>
									<label for="desc-<?php echo $q_lang . $pid; ?>">
										<img src="<?php echo trailingslashit( content_url() ) . $q_config['flag_location'] . $q_config['flag'][$q_lang]; ?>" width="18" height="12" />
										<?php echo $q_config['language_name'][$q_lang]; ?>
									</label>
									<textarea id="desc-<?php echo $q_lang . $pid; ?>" data-lang="<?php echo $q_lang; ?>" style="width:95%; margin-top: 2px;" rows="2"><?php echo qtrans_use($q_lang, stripslashes($picture->description) );  ?></textarea>
							...
							<input name="alttext[<?php echo $pid ?>]" type="text" style="width:95%; margin-bottom: 2px;<?php if( $q_translate_boxes ) : ?> display: none;<?php endif; ?>" value="<?php echo stripslashes($picture->alttext) ?>" /><br/>
							<textarea name="description[<?php echo $pid ?>]" style="width:95%; margin-top: 2px; <?php if( $q_translate_boxes ) : ?> display: none;<?php endif; ?>" rows="2" ><?php echo stripslashes($picture->description) ?></textarea>
						</td>
						<?php
					break;
					case 'exclude' :
